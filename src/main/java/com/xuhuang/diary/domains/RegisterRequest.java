...
@NoArgsConstructor
@AllArgsConstructor
@FieldMatch(fields = { "password", "passwordConfirm" },
...
public class RegisterRequest {
    ...
    @Pattern(regexp = "^.*[a-z].*$", message = VALIDATION_MESSAGE_PASSWORD_LOWER)
    @Pattern(regexp = "^.*[A-Z].*$", message = VALIDATION_MESSAGE_PASSWORD_UPPER)
    @Pattern(regexp = "^.*[0-9].*$", message = VALIDATION_MESSAGE_PASSWORD_NUMBER)
    @Pattern(regexp = "^.*[^\\w\\s].*$", message = VALIDATION_MESSAGE_PASSWORD_SPECIAL)
    @Size(min = PASSWORD_SIZE_MIN, max = PASSWORD_SIZE_MAX, message = VALIDATION_MESSAGE_PASSWORD_SIZE)
